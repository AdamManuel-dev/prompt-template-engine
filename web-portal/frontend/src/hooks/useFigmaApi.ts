...
export const useFigmaApi = (): UseFigmaApiResult => {
  ...
  const validateUrl = useCallback(
    async (url: string): Promise<FigmaUrlInfo> => {
      return handleApiCall(async () => {
        const response = await apiClient.post<FigmaUrlInfo>(
          '/api/figma/validate-url',
          {
            url,
          }
        );
  ...
  const getFileInfo = useCallback(
    async (fileId: string, forceRefresh = false): Promise<FigmaFileInfo> => {
      return handleApiCall(async () => {
  ...
  const getDesignTokens = useCallback(
    async (fileId: string, forceRefresh = false): Promise<DesignToken[]> => {
      return handleApiCall(async () => {
        const params = new URLSearchParams();
        if (forceRefresh) {
          params.set('refresh', 'true');
