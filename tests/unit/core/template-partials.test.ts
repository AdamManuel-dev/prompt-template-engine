...
describe('Template Partials', () => {
  ...
  describe('Nested partials', () => {
    ...
    it('should handle multiple levels of nesting', async () => {
      ...
      engine.registerPartial(
        'section',
        '<section><h2>{{title}}</h2>{{> list}}</section>'
      );
      ...
      const result = await engine.render(template, context);
      expect(result).toBe(
        '<section><h2>My List</h2><ul><li>Item 1</li><li>Item 2</li><li>Item 3</li></ul></section>'
      );
    });
  });
  ...
  describe('Partials with conditionals and loops', () => {
    it('should work with conditional blocks in partials', async () => {
      ...
      const loggedInResult = await engine.render(template, {
        loggedIn: true,
        username: 'Alice',
      });
      expect(loggedInResult).toBe('Welcome Alice!');

      const loggedOutResult = await engine.render(template, {
        loggedIn: false,
      });
  ...
  describe('Partials with helpers', () => {
    ...
    it('should work with complex helper expressions', async () => {
      engine.registerPartial(
        'status',
        '{{#if (gt score 70)}}Pass{{else}}Fail{{/if}}'
      );
  ...
  describe('Loading partials from files', () => {
    it('should register partial from file', async () => {
      const partialContent =
        '<nav>{{#each links}}<a href="{{url}}">{{text}}</a>{{/each}}</nav>';
  ...
  {{/if}}
      ...
      const context = {
        ...
        replies: [
          {
            text: 'First reply',
            replies: [{ text: 'Nested reply 1' }, { text: 'Nested reply 2' }],
          },
          {
            text: 'Second reply',
          },
      ...
      await expect(engine.render(template, context)).rejects.toThrow(
        'Circular partial dependency detected: comment'
      );
    });
  });
});
