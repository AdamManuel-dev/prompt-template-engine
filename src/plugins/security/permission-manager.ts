...
export class PermissionManager extends EventEmitter {
  ...
  private escalationHistory = new Map<
    string,
  ...
  async createPluginContext(
    ...
    const context: PluginSecurityContext = {
      ...
      sandboxLevel: this.determineSandboxLevel(
  ...
  async requestPermissionEscalation(
    ...
    try {
      ...
      const history = this.escalationHistory.get(pluginId) || {
        count: 0,
        lastEscalation: new Date(0),
      };
      const cooldownExpired =
        Date.now() - history.lastEscalation.getTime() >
        this.config.escalationCooldown * 1000;
      ...
      const approved = await this.evaluateEscalationRequest(
        context,
        targetTrustLevel,
        justification
      );
      ...
      await this.auditPermissionEvent('escalate', {
        ...
        context: context.executionContext,
        riskScore: this.calculateEscalationRisk(
          previousTrustLevel,
          targetTrustLevel
        ),
      });
   ...
   */
  private async loadContextPermissions(
    context: PluginSecurityContext
  ): Promise<void> {
    ...
    switch (condition.type) {
      ...
      case 'usage':
        if (condition.field === 'count' && context.executionContext) {
          fieldValue = await this.getExecutionUsageCount(
   ...
   */
  private determineSandboxLevel(
    trustLevel: PluginSecurityContext['trustLevel']
  ): PluginSecurityContext['sandboxLevel'] {
    switch (trustLevel) {
      case 'system':
        return 'none';
      case 'trusted':
