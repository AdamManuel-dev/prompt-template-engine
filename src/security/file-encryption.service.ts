...
export class FileEncryptionService {
  ...
  getEncryptionStats() {
    ...
    return {
      ...
      totalEncryptedSize: metadata.reduce(
        (sum, meta) => sum + meta.fileSize,
        0
      ),
      oldestEncryption: metadata.reduce(
        (oldest, meta) =>
          meta.encryptedAt < oldest ? meta.encryptedAt : oldest,
        now
      ),
      newestEncryption: metadata.reduce(
        (newest, meta) =>
      ...
      filesNeedingKeyRotation: metadata.filter(meta => {
        const daysSince =
          (now.getTime() - meta.encryptedAt.getTime()) / (1000 * 60 * 60 * 24);
  ...
  private shouldEncryptFile(filePath: string): boolean {
    ...
    if (this.config.encryptLogs) {
      ...
      if (logExtensions.includes(extension) && basename.includes('log'))
        return true;
    }
   ...
   */
  private async getEncryptedFiles(
    directoryPath: string,
    recursive: boolean
  ): Promise<string[]> {
    const files: string[] = [];
