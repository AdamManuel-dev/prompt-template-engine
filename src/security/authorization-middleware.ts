...
export class AuthorizationMiddleware {
  ...
  requireAnyRole(roles: string[]) {
    return async (
      ...
      return {
        ...
        error: hasAnyRole
          ? undefined
          : `One of roles [${roles.join(', ')}] required`,
      };
    };
  }
  ...
  requireOwnership(resourceIdField: string = 'id') {
    return async (
      ...
      const { user } = authResult;
      const resourceId =
        request.params?.[resourceIdField] || request.resourceId;
  ...
  protect(
    ...
    return async (
      ...
      if (options.allowRoles && options.allowRoles.length > 0) {
        const roleCheck = await this.requireAnyRole(options.allowRoles)(
          request
        );
  ...
  getStats(): {
    ...
    const successfulChecks = this.auditLog.filter(
      entry => entry.granted
    ).length;
    ...
    for (const entry of this.auditLog) {
      resourceCounts[entry.resource] =
        (resourceCounts[entry.resource] || 0) + 1;
...
export const requireAnyRole = (roles: string[]) =>
  authorizationMiddleware.requireAnyRole(roles);
