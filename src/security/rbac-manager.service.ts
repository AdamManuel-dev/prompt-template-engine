...
export class RBACManagerService {
  ...
  async deleteRole(roleId: string): Promise<boolean> {
    ...
    for (const otherRole of this.roles.values()) {
      if (otherRole.inheritsFrom?.includes(roleId)) {
        otherRole.inheritsFrom = otherRole.inheritsFrom.filter(
          id => id !== roleId
        );
  ...
  private validateRoleInheritance(
    roleId: string,
    inheritsFrom: string[]
  ): void {
  ...
  getStats(): RBACStats {
    ...
    return {
      ...
      systemRoles: Array.from(this.roles.values()).filter(r => r.isSystemRole)
        .length,
      ...
      averageEvaluationTime:
        this.evaluationStats.totalEvaluations > 0
          ? this.evaluationStats.totalTime /
            this.evaluationStats.totalEvaluations
          : 0,
    };
  }
  ...
  private initializeSystemPermissions(): void {
    const systemPermissions = [
      ...
      },
      {
        resource: 'marketplace',
        action: 'moderate',
        description: 'Moderate marketplace content',
      },
    ...
    editor.permissions = Array.from(this.permissions.values())
      .filter(
        p =>
          ...
          (p.resource === 'marketplace' && p.action !== 'moderate')
      )
      .map(p => p.id);
