...
export class TemplateSanitizer {
  ...
  sanitize(
    ...
    if (effectiveConfig.removeScripts) {
      ...
      const jsProtocolPattern = /javascript\s*:/gis;
      ...
      const vbsProtocolPattern = /vbscript\s*:/gis;
      ...
      const dataJsPattern = /data\s*:[^;]*;[^,]*(?:javascript|script)[^,]*,/gis;
    ...
    if (effectiveConfig.removeEventHandlers) {
      ...
      const eventHandlerNoQuotesPattern = /on\w+\s*=\s*[^\s>]+/gis;
      ...
      sanitized = sanitized.replace(
        ...
        (attributeMatch, attrName, attrValue) => {
          ...
          if (
            ...
            const hasValidProtocol = effectiveConfig.allowedProtocols.some(
              protocol => attrValue.toLowerCase().startsWith(protocol)
            );
    ...
    const dangerousCssPattern =
      /(expression|behavior|@import|binding|moz-binding)/gi;
    ...
    const isClean =
      threatsRemoved.length === 0 && warningsGenerated.length === 0;
  ...
  private removeAdditionalThreats(content: string): string {
    ...
    cleaned = cleaned.replace(/\$\\{[^}]*\\}/g, 'template-placeholder');
