...
const patterns = {
  ...
  sqlSafe:
    /^(?!.*(union|select|insert|delete|update|drop|create|alter|exec|execute|script|declare|cast|convert|char|varchar|nvarchar|substring|ascii|waitfor|delay|benchmark|sleep|load_file|outfile|dumpfile|information_schema|mysql|pg_|sys\.|xp_|sp_)\b).*$/i,
  // Enhanced XSS prevention with comprehensive pattern matching
  xssSafe:
    /^(?!.*(<\/?(?:script|iframe|object|embed|applet|meta|link|style|form|input|textarea|button|select|option)|javascript:|vbscript:|data:|on\w+\s*=|eval\s*\(|setTimeout|setInterval|Function\s*\(|innerHTML|outerHTML|document\.|window\.|location\.|navigator\.|XMLHttpRequest|fetch\s*\(|WebSocket|eval|unescape)).*$/i,
